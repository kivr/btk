...
class BTKbDevice:
    ...
    def interfaces_added(self, unk0, unk1):
        pass
        ...
        if self.on_disconnect is not None:
            if 'Connected' in changed:
                if not changed['Connected']:
                    match = re.search("(_[0-9A-F]{2}){6}", path)
                    address = match.group(0)[1:].replace('_',':')
                    self.on_disconnect(address)

    def on_disconnect(self, address):
        print(f'The client has been disconnect {address}')
    ...
    def listen(self):
        ...
        while True:
            ...
            self.devices.append(BTKbDeviceData(ccontrol, cinterrupt, cinfo[0]))
    ...
    def send(self, msg):
        ...
        """
        if self.current_device < len(self.devices):
            self.devices[self.current_device].interrupt.send(bytes(bytearray(msg)))
...
class BTKbService(dbus.service.Object):
    ...
    def __init__(self):
        ...
        thread = threading.Thread(target=self.device.listen, args=())
        thread.daemon = True
        thread.start()
